class AStar {
   ...
   private final void makeStar(int id) {
      ...
      int dRow = false;
      int dRow = row - 1;
      if (dRow >= 0 && this.checkMap(col, dRow)) {
         up = true;
         if (this.checkOpen(col, dRow, id) && this.checkClosed(col, dRow)) {
            this.addPoint(col, dRow, id);
         }
      }

      dRow = row + 1;
      if (dRow < Map.currentTotalRow && this.checkMap(col, dRow)) {
         down = true;
         if (this.checkOpen(col, dRow, id) && this.checkClosed(col, dRow)) {
            this.addPoint(col, dRow, id);
         }
      ...
      int dCol = col - 1;
      if (dCol >= 0 && this.checkMap(dCol, row)) {
         left = true;
         if (this.checkOpen(dCol, row, id) && this.checkClosed(dCol, row)) {
            this.addPoint(dCol, row, id);
         }
      }

      dCol = col + 1;
      if (dCol < Map.currentTotalColumn && this.checkMap(dCol, row)) {
         right = true;
         if (this.checkOpen(dCol, row, id) && this.checkClosed(dCol, row)) {
            this.addPoint(dCol, row, id);
         }
   ...
   private final boolean checkOpen(int col, int row, int fid) {
   ...
   private final boolean checkClosed(int col, int row) {
   ...
   private final boolean checkMap(int col, int row) {
      return Map.getInstance().isFloor(col, row);
   }

   private final void addPoint(int col, int row, int fId) {
   ...
   private final void resetPointResize(int i) {
      int half;
      for(int last = i; last > 1; last = half) {
         half = last >> 1;
